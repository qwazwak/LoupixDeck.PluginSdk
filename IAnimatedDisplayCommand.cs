...
/// </summary>
/// <remarks>
/// Mirrors the host's internal animation render context (minus host-only types) so a plugin
...
public readonly struct AnimationFrameContext
{
    ...
    /// <summary>
    /// Total time elapsed since the host started ticking this command.
    /// </summary>
    ...
    /// <summary>
    /// The rate the host is actually ticking at (after clamping to its global limit).
    /// </summary>
...
/// <see cref="IDisplayImageCommand"/>.
/// </summary>
/// <remarks>
/// Set <see cref="IPluginCommand.SupportedTargets"/> to <see cref="ButtonTargets.TouchButton"/>.
///
/// <para>
/// Backward compatibility: this is an independent, optional interface.
/// A command that only needs a slow periodic redraw <b>should</b> keep implementing <see cref="IDisplayImageCommand"/>;
/// implement this one only for genuine animation.
/// A command may implement both — the host prefers the animated path.
...
public interface IAnimatedDisplayCommand : IPluginCommand
{
    ...
    /// <summary>
    /// Draws the current animation frame onto <paramref name="canvas"/>.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Called off the UI thread at up to <see cref="TargetFps"/>;
    /// must be fast and synchronous (do network/decoding work
    /// asynchronously in <see cref="LoupixPlugin.Initialize"/> and cache it).
    /// </para>
    /// <para>
    /// Return <see cref="AnimationFrameInfo.Skip"/> when nothing changed (no device push),
    /// or <see cref="AnimationFrameInfo.Frame"/> / <see cref="AnimationFrameInfo.Final"/> with a
