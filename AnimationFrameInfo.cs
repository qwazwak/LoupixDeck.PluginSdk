...
/// <summary>
/// Result of a single <see cref="IAnimatedDisplayCommand.RenderAnimatedFrame"/> call. Tells the
/// host whether the plugin drew anything this tick (so the host can skip an unnecessary device
...
public readonly struct AnimationFrameInfo
{
    ...
    /// <summary>True when the plugin drew a frame this tick; false to leave the button unchanged.</summary>
    ...
    /// <summary>Nothing changed this tick — the host leaves the button unchanged.</summary>
    public static AnimationFrameInfo Skip() => new(0, drawn: false, isFinal: false);

    /// <summary>A frame numbered <paramref name="frameNumber"/> was drawn; the animation continues.</summary>
    public static AnimationFrameInfo Frame(long frameNumber) => new(frameNumber, drawn: true, isFinal: false);

    /// <summary>The final frame (<paramref name="frameNumber"/>) of a one-shot animation was drawn;
    /// the host stops ticking and holds it.</summary>
