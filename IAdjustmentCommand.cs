...
/// with a press to reset or commit.
/// Set <see cref="IPluginCommand.SupportedTargets"/> to
...
/// branching on <see cref="CommandContext.Target"/> to route them.
/// Implementing <see cref="IAdjustmentCommand"/> in addition is forward-compatible.
/// </remarks>
public interface IAdjustmentCommand : IPluginCommand
{
    /// <summary>
    /// Applies a relative change of <paramref name="ticks"/> (positive
    /// for right-turn, negative for left-turn).
    /// </summary>
    Task ApplyAdjustment(CommandContext ctx, int ticks);

    /// <summary>
    /// Resets the value (typically invoked by an encoder press).
    /// </summary>
    Task ApplyReset(CommandContext ctx);

    /// <summary>
    /// Current value as display text (e.g. "75%"), or null for no overlay.
    /// Called by the host when rendering a dial indicator.
    /// </summary>
